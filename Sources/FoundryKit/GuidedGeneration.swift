...
internal class JSONParseState {
  ...
  var currentStateKey: String {
    ...
    case .inPropertyName(let path, let partial):
      return "pname:\(path.joined(separator: ".")):\(partial)"
    case .afterPropertyName(let path, let prop):
      return "aprop:\(path.joined(separator: ".")):\(prop)"
  ...
  func getValidNextCharacters() -> Set<Character> {
    ...
    case .inNumberValue(_, _, let partial):
      ...
      if !partial.contains("e") && !partial.contains("E") && !partial.isEmpty && partial != "-" {
        validChars.insert("e")
        validChars.insert("E")
      }

      // Allow +/- after e/E for scientific notation
      if partial.hasSuffix("e") || partial.hasSuffix("E") {
        validChars.insert("+")
        validChars.insert("-")
      }

      // Can end number with comma or closing brace (if we have a valid number)
    ...
    case .inArrayNumberElement(_, _, _, _, let partial):
      ...
      // Allow +/- after e/E
      if let lastChar = partial.last, lastChar == "e" || lastChar == "E" {
        validChars.insert("+")
        validChars.insert("-")
      }

      // Allow array terminators if we have a valid number
      if isValidNumberSoFar(partial) {
        validChars.insert(",")
        validChars.insert("]")
      }
    ...
    case .inArrayBooleanElement(_, _, _, _, let partial):
      ...
      // If we have complete boolean, allow terminators
      if partial == "true" || partial == "false" {
        validChars.insert(",")
        validChars.insert("]")
      }
    ...
    case .inArrayNullElement(_, _, _, _, let partial):
      ...
      // If we have complete null, allow terminators
      if partial == "null" {
        validChars.insert(",")
        validChars.insert("]")
      }
    ...
    case .inArray(_, _, let elementType, let count):
      var validChars = Set<Character>()

      // Always allow closing the array
      validChars.insert("]")

      // If we have elements, allow comma for next element
      if count > 0 {
        validChars.insert(",")
      }
  ...
  private func processCharacter(_ char: Character) {
    ...
    case (.expectingPropertyName(let path), "\""):
      state = .inPropertyName(path: path, partial: "")
    ...
    case (.expectingPropertyName(let path), "}"):
      ...
      if path.isEmpty {
        ...
        state = .afterValue(path: Array(path.dropLast()))
      }
    ...
    case (.expectingValue(let path, let prop, .number), _) where "0123456789-".contains(char):
      state = .inNumberValue(path: path, property: prop, partial: String(char))

    case (.expectingValue(let path, let prop, .integer), _) where "0123456789-".contains(char):
      state = .inNumberValue(path: path, property: prop, partial: String(char))
    ...
    case (.expectingValue(let path, let prop, .object), "{"):
      ...
      state = .expectingPropertyName(path: newPath)
    ...
    case (.inArray(let path, let prop, .string, let count), "\""):
      // We need a new state for array string elements
      state = .inArrayStringElement(
        path: path, property: prop, elementType: .string, count: count, partial: "")

    case (.inArray(let path, let prop, .number, let count), _) where "0123456789-".contains(char):
      state = .inArrayNumberElement(
        path: path, property: prop, elementType: .number, count: count, partial: String(char))

    case (.inArray(let path, let prop, .integer, let count), _) where "0123456789-".contains(char):
      state = .inArrayNumberElement(
        path: path, property: prop, elementType: .integer, count: count, partial: String(char))
    ...
    case (.inArrayNumberElement(let path, let prop, let elemType, let count, let partial), ","),
      (.inArrayNumberElement(let path, let prop, let elemType, let count, let partial), "]"):
      // Complete number and return to array
      if isValidNumberSoFar(partial) {
        state = .inArray(path: path, property: prop, elementType: elemType, count: count + 1)
        // Process the terminator character
        processCharacter(char)
      }
    ...
    // === WHITESPACE HANDLING ===
    case (_, " "), (_, "\t"), (_, "\n"), (_, "\r"):
      // Ignore whitespace in most states (JSON allows it)
      break

    // === FALLBACK ===
    default:
      // Invalid character for current state - stay in current state
      // In a production implementation, this might set an error state
      break
    }
  }
  ...
  private func getValidPropertyNames(at path: [String]) -> Set<String> {
    ...
    for pathComponent in path {
      guard let childNode = currentNode.properties[pathComponent] else {
        // Path doesn't exist in schema, no valid properties
        return Set<String>()
      }
      currentNode = childNode
    }

    // Return all property names at current schema location
  ...
  private func isValidNumberSoFar(_ partial: String) -> Bool {
    ...
    let validPattern =
      partial.range(of: #"^-?(\d+\.?\d*|\d*\.\d+)([eE][+-]?\d+)?$"#, options: .regularExpression)
      != nil
  ...
  private func couldBeValidNumber(_ partial: String) -> Bool {
    // Check if the partial could eventually become a valid number
    if partial.isEmpty {
      return true
    }
    ...
    // Check for basic number patterns (more permissive than final validation)
    let couldBeValid =
